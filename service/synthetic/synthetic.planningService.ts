...
export const syntheticPlanningService = {
  getAllSyntheticPlanning: async (page: number, pageSize: number, status: string) => {
    ...
    try {
      ...
      const { rows, count } = await syntheticRepository.getAllSyntheticPlanning({
        page,
        pageSize,
  ...
  //get by field
  getSyntheticPlanningByFields: async ({ field, keyword, page, pageSize }: searchFieldAtribute) => {
    try {
      ...
      //query db
      const { rows } = await syntheticRepository.getAllSyntheticPlanning({
        whereCondition: {
  ...
  getAllSyntheticPlanningStage: async () => {
    try {
      const rawPapers = await syntheticRepository.exportExcelSyntheticPlanning({});
      ...
      const formatted = await Promise.all(
        rawPapers.map(async (paper) => {
          ...
          const allOverflow = await syntheticRepository.getAllTimeOverflow(box?.planningBoxId ?? 0);
  ...
  //export excel
  exportExcelSyntheticPlanning: async (req: Request, res: Response) => {
    ...
    try {
      ...
      const rawPapers = await syntheticRepository.exportExcelSyntheticPlanning({ whereCondition });
