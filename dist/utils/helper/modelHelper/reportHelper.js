...
exports.createReportPlanning = exports.getPerformanceSearchSummary = exports.getPerformanceSummaryByRows = void 0;
const sequelize_1 = require("sequelize");
const planningPaper_1 = require("../../../models/planning/planningPaper");
const dailyReportPerformance_1 = require("../../../models/report/dailyReportPerformance");
...
const calculatePerformanceSummary = (unifiedData) => {
    const summaryByDate = {};
...
const getPerformanceSummaryByRows = async (rows, machine) => {
    if (!rows || rows.length === 0)
        return {};
    const distinctDates = [
        ...new Set(rows.map((r) => new Date(r.dayReport).toISOString().split("T")[0])),
    ];
    const perfData = await dailyReportPerformance_1.DailyReportPerformance.findAll({
        where: { machine, dayReport: { [sequelize_1.Op.in]: distinctDates } },
        attributes: { exclude: ["createdAt", "updatedAt"] },
        raw: true,
    });
    // Chuẩn hóa data từ bảng phụ về dạng chung
    const unifiedData = perfData.map((item) => ({
...
exports.getPerformanceSummaryByRows = getPerformanceSummaryByRows;
// HÀM 2: Lấy trực tiếp từ kết quả Meilisearch để tính toán
const getPerformanceSearchSummary = (searchRows) => {
    if (!searchRows || searchRows.length === 0)
        return {};
    // Chuẩn hóa data từ các đơn hàng về dạng chung
    const unifiedData = searchRows.map((row) => {
...
exports.getPerformanceSearchSummary = getPerformanceSearchSummary;
// HÀM 3: Tạo báo cáo mới và cập nhật performance
const createReportPlanning = async ({ planning, model, qtyProduced, qtyWasteNorm, dayReportValue, shiftManagementBox = "", machine = "", reportedBy, totalPrice, otherData, transaction, isBox = false, }) => {
    //condition to get id
    ...
    if (isBox) {
        ...
        const prevReport = await model.findOne({
            where: { dayReport: { [sequelize_1.Op.lt]: currentDateTime } },
        ...
        const [affectedRows] = await dailyReportPerformance_1.DailyReportPerformance.update({
            totalLength: database_connect_1.sequelize.literal(`totalLength + ${parseTotalLength}`),
            totalDurations: database_connect_1.sequelize.literal(`totalDurations + ${parseDuration}`),
        }, {
            where: {
        ...
        if (affectedRows === 0) {
            await dailyReportPerformance_1.DailyReportPerformance.create({
                dayReport: (0, dayjs_config_1.dayjsUtc)(currentDateTime).toDate(),
                machine: machine,
                flute: waveLayer,
                totalLength: parseTotalLength,
